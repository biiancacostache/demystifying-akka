...
/**
  ...
  */
object CustomThreadPoolDispatcher extends App {
  class CoffeeHouseEngine(as: ActorSystem) {
      implicit val executionContext = as.dispatchers.lookup("custom-thread-pool")

      def run(requests: List[BaristaRequest]): List[Receipt] = {
        implicit val timeout = Timeout(30, TimeUnit.SECONDS)
        val taskResults = requests map { task =>
          val worker = as.actorOf(Barista.props().withDispatcher("custom-thread-pool"))
